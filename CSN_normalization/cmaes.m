...
function res = cmaes(LossFunction, nVar, VarMin, VarMax, MaxIt, fig_filename)
...
%     sigma0 = 0.3*(VarMax-VarMin);
    ...
    M(1).Position = zeros(VarSize);%unifrnd(VarMin, VarMax, VarSize);
    M(1).Step = zeros(VarSize);
    M(1).Cost = LossFunction(M(1).Position);
    ...
    for g = 1:MaxIt
        ...
        % Evaluation
        M(g+1).Cost = LossFunction(M(g+1).Position);
...
%         sigma{g+1} = sigma{g}*exp(cs/ds*(norm(ps{g+1})/ENN-1))^0.3;
        sigma{g+1} = (sigma{g}*exp(cs/ds*(norm(ps{g+1})/ENN-1))^0.3)*0.9;
